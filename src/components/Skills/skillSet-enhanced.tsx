...
const SkillBar = ({ name, level, icon, index }) => {
  // Initialize progress animation when component loads
  useEffect(() => {
    const fillElements = document.querySelectorAll(".progress-fill");

    // Create an observer to animate the progress bars when they come into view
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            const fillElement = entry.target as HTMLElement;
            const level = fillElement.getAttribute("data-level");
            setTimeout(() => {
              fillElement.style.width = `${level}%`;
            }, 300); // Delay to ensure the animation is visible
    ...
    return () => {
      fillElements.forEach((fill) => observer.unobserve(fill));
    };
  }, []);
    ...
    <div className="skill-item">
      ...
      <div className="progress-bar">
        <div
          ...
          data-level={level}
          style={{ width: "0%" }} // Start at 0 width, will be animated
        >
